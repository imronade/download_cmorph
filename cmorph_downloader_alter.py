...
def check_wget_installed():
    ...
    try:
        ...
        logging.error("wget tidak ditemukan. Pastikan wget sudah diinstal pada sistem Anda.")
        return False

def get_remote_file_size(url):
    """Get file size from remote URL using wget"""
    try:
        ...
        # Parse the output to get the Content-Length
        stderr_output = result.stderr
        for line in stderr_output.splitlines():
            if "Content-Length:" in line:
                size = line.split("Content-Length:")[1].strip().split()[0]
                return int(size)
        
        # If we get here and the command was successful, the file exists but size is unknown
        if "200 OK" in stderr_output:
            logging.warning(f"File ditemukan tetapi ukuran tidak diketahui: {url}")
            return 0
        elif "404 Not Found" in stderr_output:
            logging.warning(f"File tidak ditemukan: {url}")
            return -1
        else:
            logging.warning(f"Gagal mendapatkan ukuran file {url}")
            return -1
...
def download_file(url, local_path, max_retries=3, check_size=True):
    """Download file dari URL menggunakan wget dan simpan ke path lokal dengan percobaan ulang"""
        ...
        try:
            ...
            if result.returncode != 0:
                ...
                if "404 Not Found" in stderr_output:
                    ...
                    raise Exception(f"wget failed with return code {result.returncode}: {stderr_output}")
            ...
            if check_size:
                if not check_file_integrity(url, local_path):
                    ...
                    if retries < max_retries - 1:
                        retries += 1
                        # Hapus file yang ukurannya tidak sesuai
                        try:
                            os.remove(local_path)
                        except Exception as e:
                            logging.error(f"Gagal menghapus file {local_path}: {str(e)}")
            ...
            if retries < max_retries:
                ...
                if os.path.exists(local_path):
                    try:
                        ...
                        logging.error(f"Gagal menghapus file {local_path}: {str(e2)}")
            else:
...
def download_with_parallel_wget(start_date, end_date, num_parallel=3):
    """Mengunduh data menggunakan wget di beberapa proses secara paralel"""
    logging.info(f"Memulai download paralel dengan {num_parallel} proses")
    
    # Siapkan daftar tanggal yang akan diunduh
    dates = []
    current_date = start_date
    while current_date <= end_date:
    ...
    # Hitung total file
    total_files = len(dates) * 24  # 24 file per hari
    logging.info(f"Total akan mengunduh data untuk {len(dates)} hari ({total_files} file)")
    
    # Bagi tanggal menjadi beberapa batch untuk diproses secara paralel
    batch_size = max(1, len(dates) // num_parallel)
    batches = [dates[i:i + batch_size] for i in range(0, len(dates), batch_size)]
    
    # Buat fungsi untuk memproses satu batch tanggal
    def process_batch(batch_dates):
        batch_success = 0
        batch_failed = []
        
        for date in batch_dates:
            year, month, day = date.year, date.month, date.day
            success, failed = download_day(year, month, day)
    ...
    with concurrent.futures.ProcessPoolExecutor(max_workers=num_parallel) as executor:
        ...
        for future in tqdm(concurrent.futures.as_completed(futures), total=len(batches), desc="Batch Progress"):
            success, failed = future.result()
            successful_downloads += success
            all_failed_files.extend(failed)
    
    return successful_downloads, all_failed_files

def main():
    # Cek apakah wget terinstal
    if not check_wget_installed():
        logging.error("wget tidak ditemukan. Pastikan wget terinstal sebelum menjalankan script ini.")
        return
    ...
    # Untuk download serial (seperti versi asli)
    total_files = 0
    successful_downloads = 0
    all_failed_files = []
    ...
    # Alternatif: Uncomment untuk download paralel
    # num_parallel = 3  # Jumlah proses paralel
    # successful_downloads, all_failed_files = download_with_parallel_wget(start_date, end_date, num_parallel)
